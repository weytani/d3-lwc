...
describe("c-d3-slope-chart e2e", () => {
  ...
  describe("full lifecycle", () => {
    ...
    it("create -> load D3 -> load data -> render -> verify connecting-line SVG creation", async () => {
      ...
      expect(appendCalls.some((call) => call[0] === "line")).toBe(true);
  ...
  describe("error recovery", () => {
    it("D3 load failure -> error state -> component shows error", async () => {
      ...
      const element = await createChart({
        recordCollection: [{ Name: "Acme", Amount: 100, ExpectedRevenue: 150 }]
      });
    ...
    it("SOQL fetch path: no recordCollection -> Apex returns data -> full pipeline", async () => {
      const soqlData = [
        { Name: "Discovery Co", Amount: 400, ExpectedRevenue: 350 }
      ];
  ...
  describe("multi-component isolation", () => {
    it("two charts on same page have independent lifecycle", async () => {
      const dataA = [{ Name: "A1", Amount: 100, ExpectedRevenue: 150 }];
      const dataB = [{ Name: "B1", Amount: 300, ExpectedRevenue: 200 }];
