    ...
    const mockD3 = {
        ...
        hierarchy: jest.fn((data) => {
            const createNode = (d, parent = null, depth = 0) => {
                ...
                if (d.children) {
                    node.children = d.children.map(child => createNode(child, node, depth + 1));
                } else {
...
describe('c-d3-treemap', () => {
    ...
    describe('data handling', () => {
        ...
        it('validates hierarchyData structure', async () => {
            await createChart({
                recordCollection: [],
                hierarchyData: { name: 'Root', children: [{ name: 'Child', value: 100 }] }
            });
    ...
    describe('zoom functionality', () => {
        it('does not show breadcrumbs initially', async () => {
            ...
            const nav = element.shadowRoot.querySelector('nav[aria-label="Breadcrumbs"]');
