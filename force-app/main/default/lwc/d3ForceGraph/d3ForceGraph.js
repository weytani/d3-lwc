/**
 * D3 Force Graph Lightning Web Component
 * Displays network/graph visualization with nodes and edges using force simulation.
 * Supports drag interaction, zoom/pan, tooltips, and click-to-navigate.
 */
...
export default class D3ForceGraph extends NavigationMixin(LightningElement) {
    ...
    async loadData() {
        ...
        if (this.recordCollection && this.recordCollection.length > 0) {
            ...
            throw new Error('No data source provided. Set recordCollection, graphData, or soqlQuery.');
        }
    ...
    validateGraphData(data) {
        ...
        const nodes = data.nodes.slice(0, MAX_NODES).map((node, index) => ({
            ...
            label: node.label || node[this.nodeLabelField] || node.name || `Node ${index}`,
    ...
    buildGraphData(data) {
        ...
        data.forEach(record => {
            ...
            links.push({
                source: sourceId,
                target: targetId,
                weight: this.linkWeightField ? Number(record[this.linkWeightField]) || 1 : 1
            });
        });
    ...
    renderChart(containerWidth) {
        ...
        const colors = getColors(this.theme, Math.max(colorCount, 1), this.config.customColors);
        
        const colorScale = d3.scaleOrdinal()
            .domain(this._nodeGroups)
            .range(colors);
        ...
        this.simulation = d3.forceSimulation(nodes)
            ...
            .force('collision', d3.forceCollide()
                .radius(d => this.getNodeRadius(d, radiusScale) * this.collisionMultiplier));
        ...
        const link = linkGroup.selectAll('line')
            ...
            .join('line')
            .attr('class', 'link')
            .attr('stroke-width', d => this.linkWeightField ? linkWidthScale(d.weight) : this.linkWidth);
    ...
    resetHighlights(nodeSelection, linkSelection) {
        ...
        linkSelection
            ...
            .duration(100)
            .attr('stroke-opacity', this.linkOpacity)
            .attr('stroke-width', d => this.linkWeightField ? d.weight : this.linkWidth);
    }
