...
export default class D3VariableColorLine extends NavigationMixin(
  LightningElement
) {
  // ═══════════════════════════════════════════════════════════════
   ...
   * are set. This chart has no server-side aggregate: it always fetches raw records
   * for dateField and valueField, then feeds the existing processTimeSeriesData path
   * (same as recordCollection/soqlQuery).
   */
  get gqlQuery() {
  ...
  async connectedCallback() {
    try {
      ...
      console.error("D3VariableColorLine initialization error:", e);
    } finally {
      this.isLoading = false;
    }
  }
