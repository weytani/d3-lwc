// ABOUTME: Tests the additive GraphQL self-fetch path on d3Heatmap (Approach A, CT-MG).
// ABOUTME: Sum/Average/Min/Max use buildMultiGroupQuery/normalizeMultiGroup (Task 1's
// ABOUTME: live-verified two-field aggregate envelope). Count has no server aggregate, so it
// ABOUTME: falls back to a bounded raw-record fetch feeding the existing _aggregateRawData path.
import { createElement } from "lwc";
import D3Heatmap from "c/d3Heatmap";
import { graphql, gql } from "lightning/graphql";
import { loadD3 } from "c/d3Lib";

jest.mock("c/d3Lib", () => ({ loadD3: jest.fn() }));

// Hand-rolled mock D3 (mirrors d3Heatmap.test.js): extent/max/min are real
// jest.fn() implementations returning fixed numbers, not a naive Proxy, so
// there is no thenable trap and no risk of primitive-conversion crashes.
const createMockD3 = () => {
  const mockD3 = {
    select: jest.fn(() => mockD3),
    append: jest.fn(() => mockD3),
    attr: jest.fn(() => mockD3),
    style: jest.fn(() => mockD3),
    call: jest.fn(() => mockD3),
    insert: jest.fn(() => mockD3),
    text: jest.fn(() => mockD3),
    remove: jest.fn(() => mockD3),
    selectAll: jest.fn(() => mockD3),
    data: jest.fn(() => mockD3),
    enter: jest.fn(() => mockD3),
    exit: jest.fn(() => mockD3),
    transition: jest.fn(() => mockD3),
    duration: jest.fn(() => mockD3),
    delay: jest.fn(() => mockD3),
    on: jest.fn(() => mockD3),
    merge: jest.fn(() => mockD3),
    html: jest.fn(() => mockD3),
    each: jest.fn(() => mockD3),
    scaleBand: jest.fn(() => {
      const scale = jest.fn(() => 50);
      scale.domain = jest.fn(() => scale);
      scale.range = jest.fn(() => scale);
      scale.padding = jest.fn(() => scale);
      scale.bandwidth = jest.fn(() => 40);
      return scale;
    }),
    scaleQuantize: jest.fn(() => {
      const scale = jest.fn(() => "#1589EE");
      scale.domain = jest.fn(() => scale);
      scale.range = jest.fn(() => scale);
      return scale;
    }),
    axisBottom: jest.fn(() => {
      const axis = jest.fn();
      axis.tickFormat = jest.fn(() => axis);
      axis.tickSize = jest.fn(() => axis);
      return axis;
    }),
    axisLeft: jest.fn(() => {
      const axis = jest.fn();
      axis.tickFormat = jest.fn(() => axis);
      axis.tickSize = jest.fn(() => axis);
      return axis;
    }),
    min: jest.fn(() => 0),
    max: jest.fn(() => 500),
    extent: jest.fn(() => [0, 500])
  };
  return mockD3;
};

const AGG_RESPONSE = {
  uiapi: {
    aggregate: {
      Opportunity: {
        edges: [
          {
            node: {
              aggregate: {
                Quarter: { value: "Q1" },
                Product: { value: "Product A" },
                Amount: { sum: { value: 100 } }
              }
            }
          },
          {
            node: {
              aggregate: {
                Quarter: { value: "Q1" },
                Product: { value: "Product B" },
                Amount: { sum: { value: 200 } }
              }
            }
          }
        ]
      }
    }
  }
};

const RECORD_RESPONSE = {
  uiapi: {
    query: {
      Opportunity: {
        edges: [
          { node: { Quarter: { value: "Q1" }, Product: { value: "A" } } },
          { node: { Quarter: { value: "Q1" }, Product: { value: "B" } } },
          { node: { Quarter: { value: "Q2" }, Product: { value: "A" } } }
        ]
      }
    }
  }
};

async function flushPromises() {
  return Promise.resolve();
}

describe("d3Heatmap GraphQL path (Approach A, CT-MG)", () => {
  let mockD3;

  beforeEach(() => {
    mockD3 = createMockD3();
    loadD3.mockResolvedValue(mockD3);

    Element.prototype.getBoundingClientRect = jest.fn(() => ({
      width: 600,
      height: 400,
      top: 0,
      left: 0,
      bottom: 400,
      right: 600
    }));

    global.ResizeObserver = jest.fn().mockImplementation(() => ({
      observe: jest.fn(),
      unobserve: jest.fn(),
      disconnect: jest.fn()
    }));
  });

  afterEach(() => {
    while (document.body.firstChild)
      document.body.removeChild(document.body.firstChild);
    jest.clearAllMocks();
  });

  it("renders the chart container and actually draws cells when GraphQL aggregate data arrives", async () => {
    const element = createElement("c-d3-heatmap", { is: D3Heatmap });
    element.fetchMode = "graphql";
    element.objectApiName = "Opportunity";
    element.xField = "Quarter";
    element.yField = "Product";
    element.valueField = "Amount";
    element.operation = "Sum";
    document.body.appendChild(element);

    await flushPromises(); // connectedCallback (loadD3 + loadData early-return)
    graphql.emit(AGG_RESPONSE);
    await flushPromises();
    await flushPromises();

    expect(element.shadowRoot.querySelector(".chart-container")).not.toBeNull();
    expect(
      element.shadowRoot.querySelector(".slds-text-color_error")
    ).toBeNull();

    // Prove renderChart actually ran (not just that the wire populated data):
    // a "heatmap-cell" must have been appended.
    expect(
      mockD3.attr.mock.calls.some(
        (c) => c[0] === "class" && c[1] === "heatmap-cell"
      )
    ).toBe(true);
  });

  it("shows an error when the GraphQL wire emits errors", async () => {
    const element = createElement("c-d3-heatmap", { is: D3Heatmap });
    element.fetchMode = "graphql";
    element.objectApiName = "Opportunity";
    element.xField = "Quarter";
    element.yField = "Product";
    element.valueField = "Amount";
    element.operation = "Sum";
    document.body.appendChild(element);

    await flushPromises();
    graphql.emitErrors([{ message: "boom" }]);
    await flushPromises();

    expect(
      element.shadowRoot.querySelector(".slds-text-color_error")
    ).not.toBeNull();
  });

  it("bounds the query with the same first: value as other CT-MG charts", async () => {
    const element = createElement("c-d3-heatmap", { is: D3Heatmap });
    element.fetchMode = "graphql";
    element.objectApiName = "Opportunity";
    element.xField = "Quarter";
    element.yField = "Product";
    element.valueField = "Amount";
    element.operation = "Sum";
    document.body.appendChild(element);

    await flushPromises();

    const queryStrings = gql.mock.results.map((r) => r.value);
    expect(queryStrings.some((q) => q.includes("first: 2000"))).toBe(true);
  });

  it("builds a groupBy on both xField and yField for Sum", async () => {
    const element = createElement("c-d3-heatmap", { is: D3Heatmap });
    element.fetchMode = "graphql";
    element.objectApiName = "Opportunity";
    element.xField = "Quarter";
    element.yField = "Product";
    element.valueField = "Amount";
    element.operation = "Sum";
    document.body.appendChild(element);

    await flushPromises();

    const queryStrings = gql.mock.results.map((r) => r.value);
    const query = queryStrings[queryStrings.length - 1];
    expect(query).toContain("groupBy: { Quarter: {}, Product: {} }");
    expect(query).toContain("sum { value }");
  });

  it("falls back to a bounded raw-record fetch for Count (no server aggregate)", async () => {
    const element = createElement("c-d3-heatmap", { is: D3Heatmap });
    element.fetchMode = "graphql";
    element.objectApiName = "Opportunity";
    element.xField = "Quarter";
    element.yField = "Product";
    element.operation = "Count";
    document.body.appendChild(element);

    await flushPromises();
    graphql.emit(RECORD_RESPONSE);
    await flushPromises();
    await flushPromises();

    expect(element.shadowRoot.querySelector(".chart-container")).not.toBeNull();
    expect(
      element.shadowRoot.querySelector(".slds-text-color_error")
    ).toBeNull();

    // The Count query is a raw record fetch, not an aggregate: uiapi.query, not uiapi.aggregate.
    const queryStrings = gql.mock.results.map((r) => r.value);
    const query = queryStrings[queryStrings.length - 1];
    expect(query).toContain("uiapi { query {");
    expect(query).not.toContain("uiapi { aggregate {");

    // Prove renderChart actually ran with the Count-derived cell grid.
    expect(
      mockD3.attr.mock.calls.some(
        (c) => c[0] === "class" && c[1] === "heatmap-cell"
      )
    ).toBe(true);
  });

  it("does not require valueField for Count", async () => {
    const element = createElement("c-d3-heatmap", { is: D3Heatmap });
    element.fetchMode = "graphql";
    element.objectApiName = "Opportunity";
    element.xField = "Quarter";
    element.yField = "Product";
    element.valueField = "";
    element.operation = "Count";
    document.body.appendChild(element);

    await flushPromises();

    expect(gql).toHaveBeenCalled();
  });

  it("does not provision the wire when valueField is missing for Sum", async () => {
    const element = createElement("c-d3-heatmap", { is: D3Heatmap });
    element.fetchMode = "graphql";
    element.objectApiName = "Opportunity";
    element.xField = "Quarter";
    element.yField = "Product";
    element.valueField = "";
    element.operation = "Sum";
    document.body.appendChild(element);

    await flushPromises();

    expect(gql).not.toHaveBeenCalled();
  });
});
