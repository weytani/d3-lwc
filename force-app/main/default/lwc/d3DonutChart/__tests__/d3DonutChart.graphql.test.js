...
// while recording the (attr/append/…) calls the renderer makes so tests can
// assert a real slice mark was drawn. The `then` guard keeps this from
// looking like a thenable to Promise.resolve()/await — without it,
// `prop === "then"` would return a callable that swallows (resolve, reject),
// and awaiting loadD3() would hang forever.
function makeD3Stub() {
  const calls = [];
...
describe("d3DonutChart GraphQL path", () => {
  ...
  it("renders the chart container and draws a real slice when GraphQL aggregate data arrives", async () => {
    ...
    // a "slice" group must have been appended.
    expect(
      d3Calls.some(
        (c) => c[0] === "attr" && c[1] === "class" && c[2] === "slice"
      )
    ).toBe(true);
  });
