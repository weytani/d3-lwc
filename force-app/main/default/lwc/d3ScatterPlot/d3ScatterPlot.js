/**
 * D3 Scatter Plot Lightning Web Component
 * Displays correlation between two numeric fields with optional trend line.
 * Points are clickable to navigate to individual records.
 */
import { LightningElement, api, track } from 'lwc';
import { loadD3 } from 'c/d3Lib';
import { prepareData } from 'c/dataService';
import { getColors, DEFAULT_THEME } from 'c/themeService';
import { formatNumber, createTooltip, createResizeHandler } from 'c/chartUtils';
...
export default class D3ScatterPlot extends NavigationMixin(LightningElement) {
    ...
    get showCorrelationInfo() {
        return (this.showTrendLine || this.showCorrelation) && this.correlationCoefficient !== null;
    }
    ...
    processScatterData(data) {
        ...
        this.chartData = data
            .map(record => {
                ...
                const group = this.groupByField ? String(record[this.groupByField] || 'Other') : 'default';
    ...
    renderChart(containerWidth) {
        ...
        if (this.showTrendLine && this.chartData.length >= 2) {
            this.drawTrendLine(xScale, yScale, width);
        }
        ...
        if (this.groupByField && this._groupNames.length > 1) {
            this.renderLegend(colorScale, width, height);
        }
    }
     ...
     */
    drawTrendLine(xScale, yScale, width) {
     ...
     */
    renderLegend(colorScale, width, height) {
