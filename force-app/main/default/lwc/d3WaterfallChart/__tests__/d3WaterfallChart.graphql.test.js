// ABOUTME: Tests the additive GraphQL self-fetch path on d3WaterfallChart (Approach A).
...
// except max()/min(), which renderChart uses synchronously ((yMax - yMin) *
// 0.1) before any further D3 chaining and so need real numbers back, not the
// chain object.
...
function makeD3Stub() {
  const chain = new Proxy(function () {}, {
    get: (target, prop) => {
      ...
      if (prop === "min")
        return (arr, accessor) => Math.min(...arr.map(accessor ?? ((d) => d)));
      return () => chain;
    },
    apply: () => chain
  });
...
describe("d3WaterfallChart GraphQL path (Approach A)", () => {
  beforeEach(() => {
  ...
  it("renders the chart container when GraphQL aggregate data arrives", async () => {
    const element = createElement("c-d3-waterfall-chart", {
      is: D3WaterfallChart
    });
