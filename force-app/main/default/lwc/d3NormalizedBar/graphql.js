// ABOUTME: Bundle-local Salesforce GraphQL (v2) query builders and normalizers for the d3NormalizedBar bundle.
 ...
 * Used by both the Count path (project the group-by + series fields, then
 * aggregate client-side) and the free-text `graphqlQuery` override (project the
 * field mappings). When `objectApiName` is omitted or absent from the payload,
 * the first object key under `data.uiapi.query` is used — so an admin's free-text
...
/**
 * Builds a two-field grouped-aggregate GraphQL query string
 * (groupBy on both the category field and the series field).
 ...
 * [{label,series,value}] for the normalized (100%) stacked bar chart.
