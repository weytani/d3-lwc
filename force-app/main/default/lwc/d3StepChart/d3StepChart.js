...
export default class D3StepChart extends NavigationMixin(LightningElement) {
  // ═══════════════════════════════════════════════════════════════
   ...
   * are set. Step has no server-side aggregate: it always fetches raw records for
  ...
  async connectedCallback() {
    try {
      ...
      console.error("D3StepChart initialization error:", e);
    } finally {
      this.isLoading = false;
    }
  }
  ...
  async loadData() {
    ...
    // Required fields for step chart
