// ABOUTME: Bundle-local Salesforce GraphQL (v2) query builders and normalizers for the d3WaffleChart bundle.
 ...
 * Used by both the Count path (project the group-by field, then aggregate
 * client-side) and the free-text `graphqlQuery` override (project the group-by
 * and value fields). When `objectApiName` is omitted or absent from the
 * payload, the first object key under `data.uiapi.query` is used — so an
 * admin's free-text query is accepted regardless of the object it targets.
 * When `fields` is omitted, every field present on the node is projected.
...
/**
 * Normalizes a grouped-aggregate wire result into [{label,value}] for the waffle chart.
