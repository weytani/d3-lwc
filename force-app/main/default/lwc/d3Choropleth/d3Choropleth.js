...
export default class D3Choropleth extends NavigationMixin(LightningElement) {
  ...
  async loadGeoData() {
    if (this.mapType === "custom" && this.geoJsonData) {
      ...
      throw new Error(
        'World map requires custom GeoJSON data. Set mapType to "custom" and provide geoJsonData.'
      );
    } else {
    ...
    if (
      ...
      this.geoData.features.length === 0
    ) {
  ...
  buildRegionLookup() {
    ...
    this.geoData.features.forEach((feature) => {
      ...
      const name =
        feature.properties?.[this.geoJsonNameProperty] ||
  ...
  renderChart(containerWidth) {
    ...
    const padding = Math.max(10, Math.round(containerWidth * 0.02));
    const margin = {
      top: padding,
      right: padding,
      bottom: padding,
    ...
    if (
      this.effectiveShowLegend &&
      this._valueExtent[1] > this._valueExtent[0]
    ) {
  ...
  createProjection(width, height, margin) {
    ...
    if (this.mapType === "us-states") {
      ...
      return d3.geoAlbersUsa().fitSize([innerWidth, innerHeight], this.geoData);
    }
    ...
    return d3.geoMercator().fitSize([innerWidth, innerHeight], this.geoData);
  }
  ...
  showRegionTooltip(event, d) {
    ...
    const regionName =
      d.properties?.[this.geoJsonNameProperty] ||
      d.properties?.name ||
      d.id ||
      "Unknown";
  ...
  handleRegionClick(event, d) {
    const regionName =
      d.properties?.[this.geoJsonNameProperty] || d.properties?.name || d.id;
    const regionId = d.id || d.properties?.[this.geoJsonIdProperty];
    const data = this.getRegionData(d);

    // Dispatch custom event
    this.dispatchEvent(
      new CustomEvent("regionclick", {
        detail: {
          regionId: regionId,
  ...
  renderLegend(svg, width, height) {
    ...
    for (let i = 0; i <= numStops; i++) {
      ...
      const value =
        this._valueExtent[0] +
        t * (this._valueExtent[1] - this._valueExtent[0]);
      gradient
        .append("stop")
        .attr("offset", `${t * 100}%`)
  ...
  addZoomControls(svg, g, width) {
    ...
    const zoomIn = controls
      ...
      .attr("cursor", "pointer")
      .on("click", () => {
        svg.transition().duration(300).call(this.zoomBehavior.scaleBy, 1.5);
      });
    ...
    const zoomOut = controls
      ...
      .attr("cursor", "pointer")
      .on("click", () => {
        svg.transition().duration(300).call(this.zoomBehavior.scaleBy, 0.67);
      });
    ...
    const reset = controls
      ...
      .on("click", () => {
        svg
          .transition()
          .duration(300)
          .call(this.zoomBehavior.transform, this.d3.zoomIdentity);
