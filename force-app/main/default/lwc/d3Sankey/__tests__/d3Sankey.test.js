...
// Mock loadScript for plugin loading
const mockLoadScript = jest.fn().mockResolvedValue();
jest.mock(
...
// Mock sankey static resource
jest.mock("@salesforce/resourceUrl/d3Sankey", () => "mockD3SankeyResourceUrl", {
  virtual: true
});
...
describe("c-d3-sankey", () => {
  ...
  describe("configuration", () => {
    ...
    it("parses advancedConfig JSON correctly", async () => {
      await createChart({
        advancedConfig:
          '{"customColors": ["#FF0000", "#00FF00"], "linkColor": "#CCC"}'
      });
  ...
  // ═══════════════════════════════════════════════════════════════
  // RESPONSIVE LABEL MARGIN TESTS
  // ═══════════════════════════════════════════════════════════════

  describe("responsive label margins", () => {
    // Extracts the left margin from the SVG g-element translate transform
    function getTranslateMargin() {
      const transformCalls = mockD3.attr.mock.calls.filter(
        ([key, val]) =>
          key === "transform" &&
          typeof val === "string" &&
          val.startsWith("translate(")
      );
      if (transformCalls.length === 0) return null;
      const match = transformCalls[0][1].match(/translate\((\d+(?:\.\d+)?),/);
      return match ? parseFloat(match[1]) : null;
    }

    it("uses dynamic margin smaller than old hardcoded 120 for short labels", async () => {
      await createChart({
        sourceField: "From",
        targetField: "To",
        valueField: "Value",
      ...
      const margin = getTranslateMargin();
      expect(margin).not.toBeNull();
      expect(margin).toBeLessThan(120);
      expect(margin).toBeGreaterThanOrEqual(20);
    });

    it("produces wider margin for longer labels", async () => {
      await createChart({
        sourceField: "From",
        targetField: "To",
        valueField: "Value",
      ...
      await Promise.resolve();
      const shortMargin = getTranslateMargin();
      ...
      await Promise.resolve();
      const longMargin = getTranslateMargin();

      expect(longMargin).toBeGreaterThan(shortMargin);
    });

    it("caps margin at 25% of container width", async () => {
      Element.prototype.getBoundingClientRect = jest.fn(() => ({
        width: 200,
        height: 400,
        top: 0,
      ...
      await createChart({
        ...
        recordCollection: [
          {
            Id: "001",
            From: "Very Long Source Name",
            To: "Very Long Target Name",
            Value: 100
          }
        ]
      });
    ...
    it("returns minimal margin when labels are inside", async () => {
      await createChart({ labelPosition: "inside" });
      await Promise.resolve();
    ...
    it("returns minimal margin when labels are hidden", async () => {
    ...
    it("accounts for value suffix in margin width", async () => {
      await createChart({
        sourceField: "From",
        targetField: "To",
        valueField: "Value",
        showValues: false,
        recordCollection: [
          { Id: "001", From: "Sales", To: "Revenue", Value: 1000000 }
        ]
      });
      await Promise.resolve();
      const marginNoValues = getTranslateMargin();
      ...
      await Promise.resolve();
      const marginWithValues = getTranslateMargin();

      expect(marginWithValues).toBeGreaterThan(marginNoValues);
    });
  });

  // ═══════════════════════════════════════════════════════════════
  // SANKEY PLUGIN LOADING TESTS
  // ═══════════════════════════════════════════════════════════════

  describe("sankey plugin loading", () => {
    it("loads sankey plugin when d3.sankey is not present", async () => {
      const d3WithoutSankey = createMockD3();
      delete d3WithoutSankey.sankey;
      delete d3WithoutSankey.sankeyJustify;
      delete d3WithoutSankey.sankeyLinkHorizontal;
      loadD3.mockResolvedValue(d3WithoutSankey);

      // Simulate plugin attaching to d3 after loadScript
      mockLoadScript.mockImplementation(() => {
        d3WithoutSankey.sankey = createMockD3().sankey;
        d3WithoutSankey.sankeyJustify = jest.fn();
        d3WithoutSankey.sankeyLinkHorizontal = jest.fn(() =>
          jest.fn(() => "M0,0 L100,100")
        );
    ...
    it("skips plugin load when d3.sankey is already present", async () => {
      // Default mockD3 already has sankey
      await createChart();
      await Promise.resolve();
    ...
    it("shows error when sankey plugin fails to load", async () => {
      const d3WithoutSankey = createMockD3();
      delete d3WithoutSankey.sankey;
      delete d3WithoutSankey.sankeyJustify;
      delete d3WithoutSankey.sankeyLinkHorizontal;
      ...
      mockLoadScript.mockRejectedValue(new Error("Plugin load failed"));
