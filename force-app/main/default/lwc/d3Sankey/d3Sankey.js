/**
 * D3 Sankey Lightning Web Component
 * Displays flow/process visualization with nodes and links.
 * Supports Source → Target → Value data structure with configurable styling.
 */
...
export default class D3Sankey extends NavigationMixin(LightningElement) {
    ...
    async loadData() {
        ...
        if (this.recordCollection && this.recordCollection.length > 0) {
            ...
            throw new Error('No data source provided. Set recordCollection, sankeyData, or soqlQuery.');
        }
    ...
    calculateTotalValue() {
        ...
        this.totalValue = this.graphData.links.reduce((sum, link) => sum + (link.value || 0), 0);
    }
    ...
    renderChart(containerWidth) {
        ...
        const margin = {
            top: 10,
            right: this.labelPosition === 'outside' ? 120 : 20,
            bottom: 10,
            left: this.labelPosition === 'outside' ? 120 : 20
        };
        ...
        // Check if d3.sankey exists (it's a separate module)
        if (!d3.sankey) {
            console.error('D3 Sankey module not available');
            this.error = 'D3 Sankey module not loaded';
            return;
        }
        ...
        // Get colors
        const colors = getColors(this.theme, nodes.length, this.config.customColors);
        const colorMap = new Map();
        nodes.forEach((node, i) => colorMap.set(node.name, colors[i % colors.length]));
        ...
        if (this.effectiveShowLabels) {
            this.drawLabels(nodes, width, colorMap);
        }
    }
     ...
     * @param {Map} colorMap - Node name to color mapping
     */
    drawLabels(nodes, width, colorMap) {
        ...
        nodes.forEach(node => {
            ...
            const x = labelPosition === 'inside' 
                ? node.x0 + (node.x1 - node.x0) / 2
                : (isLeftSide ? node.x0 - 6 : node.x1 + 6);
            
            const y = (node.y0 + node.y1) / 2;

            const anchor = labelPosition === 'inside' 
                ? 'middle' 
    ...
    highlightConnectedLinks(node, links, linkPaths) {
        ...
        linkPaths.each((d, i, elements) => {
            ...
            d3.select(elements[i])
                .transition()
                .duration(100)
                .attr('stroke-opacity', isConnected ? this.linkHoverOpacity : this.linkOpacity * 0.3);
        });
    }
