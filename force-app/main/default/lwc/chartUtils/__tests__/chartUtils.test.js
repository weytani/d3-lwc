...
describe("chartUtils", () => {
  ...
  describe("createTooltip", () => {
    ...
    it("has SLDS classes", () => {
      ...
      expect(tooltip.element.classList.contains("slds-popover_tooltip")).toBe(
        true
      );
    });
    ...
    it("show() displays tooltip with content", () => {
      ...
      expect(
        tooltip.element.querySelector(".slds-popover__body").innerHTML
      ).toContain("Test");
  ...
  describe("buildTooltipContent", () => {
    ...
    it("uses custom formatter", () => {
      const html = buildTooltipContent("Count", 42, {
    ...
    it("adds prefix and suffix", () => {
      const html = buildTooltipContent("Amount", 100, {
        prefix: "$",
        suffix: " USD"
      });
  ...
  describe("createResizeHandler", () => {
    ...
    it("calls callback with dimensions on observe", () => {
  ...
  describe("calculateDimensions", () => {
    ...
    it("returns non-negative dimensions", () => {
      const result = calculateDimensions(50, 50, {
        top: 100,
        right: 0,
        bottom: 0,
        left: 0
  ...
  describe("createLayoutRetry", () => {
    ...
    // Helper: fire the oldest pending RAF callback
    ...
    it("calls onLayout with width when container has non-zero width on first frame", () => {
      const container = { getBoundingClientRect: () => ({ width: 400 }) };
      ...
      expect(onLayout).toHaveBeenCalledWith(400);
      expect(onLayout).toHaveBeenCalledTimes(1);
    });

    it("retries when width is 0, calls onLayout when width becomes non-zero", () => {
      let width = 0;
      const container = { getBoundingClientRect: () => ({ width }) };
      const onLayout = jest.fn();

      createLayoutRetry(container, onLayout);

      // First frame: width is 0
      flushNextRAF();
      expect(onLayout).not.toHaveBeenCalled();

      // Second frame: still 0
      flushNextRAF();
      expect(onLayout).not.toHaveBeenCalled();

      // Third frame: container gets width
      width = 400;
      flushNextRAF();
      expect(onLayout).toHaveBeenCalledWith(400);
      expect(onLayout).toHaveBeenCalledTimes(1);
    ...
    it("gives up after maxAttempts without calling onLayout", () => {
      const container = { getBoundingClientRect: () => ({ width: 0 }) };
      const onLayout = jest.fn();

      createLayoutRetry(container, onLayout, { maxAttempts: 3 });

      // Fire all 3 attempts (attempt 0, 1, 2) plus the initial RAF
      for (let i = 0; i <= 3; i++) {
        flushNextRAF();
      }

      expect(onLayout).not.toHaveBeenCalled();
      // No more RAFs scheduled
    ...
    it("cancel() stops the retry loop", () => {
      ...
      const retry = createLayoutRetry(container, onLayout);
      ...
      // Width becomes available but we cancelled
      width = 400;
      flushNextRAF();
      expect(onLayout).not.toHaveBeenCalled();
    });

    it("cancel() during pending RAF does not call onLayout", () => {
      const container = { getBoundingClientRect: () => ({ width: 400 }) };
      const onLayout = jest.fn();

      const retry = createLayoutRetry(container, onLayout);

      // Cancel before the first RAF fires
      retry.cancel();

      expect(global.cancelAnimationFrame).toHaveBeenCalled();

      // Try to flush — the callback was cancelled
      flushNextRAF();
      expect(onLayout).not.toHaveBeenCalled();
    });

    it("does not call onLayout if already cancelled before first frame", () => {
      const container = { getBoundingClientRect: () => ({ width: 400 }) };
      const onLayout = jest.fn();
      ...
      // Even if someone manually fires remaining callbacks
      rafCallbacks.forEach((cb) => cb());

      expect(onLayout).not.toHaveBeenCalled();
    });

    it("uses default maxAttempts of 60", () => {
      const container = { getBoundingClientRect: () => ({ width: 0 }) };
      const onLayout = jest.fn();

      createLayoutRetry(container, onLayout);

      // Flush 61 frames (0 through 60)
      for (let i = 0; i <= 60; i++) {
        flushNextRAF();
      }
