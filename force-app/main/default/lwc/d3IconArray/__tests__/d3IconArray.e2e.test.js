// ABOUTME: End-to-end lifecycle tests for the D3 Icon Array component.
// ABOUTME: Verifies full render pipeline, 100-icon allocation, multi-instance isolation, and error recovery using real services with mocked D3.
...
describe("c-d3-icon-array e2e", () => {
  ...
  describe("full render lifecycle", () => {
    it("creates icon array end-to-end with 100 icons and pristine console", async () => {
  ...
  describe("data-flow verification", () => {
    it("real Sum aggregation flows into exact icon counts end-to-end", async () => {
      const mockD3 = createMockD3();
      loadD3.mockResolvedValue(mockD3);
      ...
      const icons = mockD3.data.mock.calls.find(
        (call) => Array.isArray(call[0]) && call[0].length === 100
      )[0];
      ...
      // round(53), round(32), round(16)->trimmed to 15 by cap = 100 filled icons
  ...
  describe("multi-component isolation", () => {
    it("two instances render independently with separate D3 state", async () => {
      ...
      // Each instance bound its own 100-icon array
      const iconsA = mockD3A.data.mock.calls.find(
        (call) => Array.isArray(call[0]) && call[0].length === 100
      )[0];
      const iconsB = mockD3B.data.mock.calls.find(
        (call) => Array.isArray(call[0]) && call[0].length === 100
      )[0];

      expect(iconsA.length).toBe(100);
      expect(iconsB.length).toBe(100);

      // Theme isolation: instance A uses Salesforce Standard, B uses Warm
      const closedWonA = iconsA.find((c) => c.label === "Closed Won");
      const closedWonB = iconsB.find((c) => c.label === "Closed Won");
