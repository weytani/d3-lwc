// ABOUTME: D3 icon array (pictogram/unit chart) Lightning Web Component.
// ABOUTME: Each category fills round(proportion*100) of 100 icon glyphs; colors via themeService, contrast via chartUtils.
...
/** Maps an advancedConfig.iconShape string to a d3.symbol type constant. */
function resolveSymbolType(d3, shape) {
  switch (shape) {
    case "square":
...
export default class D3IconArray extends NavigationMixin(LightningElement) {
  // ═══════════════════════════════════════════════════════════════
  ...
  /** Field to group by (icon categories) */
  @api groupByField = "";
  ...
  /** Grid columns for the icon layout (default 10, clamped to [1, TOTAL_CELLS]). */
  get effectiveColumns() {
    const configured = parseInt(this.config.columns, 10);
    if (!configured || configured < 1) return DEFAULT_COLUMNS;
    return Math.min(configured, TOTAL_CELLS);
  }
   ...
   * Count is this chart's PRIMARY graphql path (default operation).
   */
  get gqlQuery() {
  ...
  async connectedCallback() {
    try {
      ...
      console.error("D3IconArray initialization error:", e);
    } finally {
      this.isLoading = false;
    }
  }
  ...
  /**
   * Allocates the 100 icon slots across categories in descending value order.
   * Each category gets round(proportion * 100) icons; the running total is
  ...
  /**
   * Expands category allocations into a flat array of exactly 100 icon
   * descriptors, each carrying its grid row/column, label, value, and color.
   * @param {Array} allocations - output of _allocateIcons()
   * @param {Number} columns - grid column count
   * @returns {Array<{index:number,row:number,col:number,rows:number,label:string,value:number,color:string,textColor:string}>}
   */
  _buildIcons(allocations, columns) {
    const icons = [];
    ...
    while (icons.length < TOTAL_CELLS) {
      icons.push({
        label: null,
        value: 0,
        color: "#E5E5E5",
    ...
    const rows = Math.ceil(TOTAL_CELLS / columns);
    return icons.slice(0, TOTAL_CELLS).map((icon, index) => ({
      ...icon,
      index,
  ...
  renderChart(containerWidth) {
    ...
    const rows = Math.ceil(TOTAL_CELLS / columns);
    const gap = Math.min(width / columns, height / rows) * 0.15;
    const cellWidth = (width - gap * (columns - 1)) / columns;
    const cellHeight = (height - gap * (rows - 1)) / rows;
    const cellDim = Math.max(1, Math.min(cellWidth, cellHeight));

    // Center the grid within the (typically wider) drawing area so it
    // doesn't hug the left edge leaving empty space on the right.
    const gridWidth = columns * cellWidth + (columns - 1) * gap;
    const gridHeight = rows * cellHeight + (rows - 1) * gap;
    const offsetX = margin.left + Math.max(0, (width - gridWidth) / 2);
    const offsetY = margin.top + Math.max(0, (height - gridHeight) / 2);
    ...
    const svgRoot = d3
      ...
      .attr("class", "icon-array-svg");

    applySvgA11y(svgRoot, {
      title: `Icon array: ${this.operation} of ${this.valueField || this.groupByField} by ${this.groupByField}`,
    ...
    const allocations = this._allocateIcons();
    const icons = this._buildIcons(allocations, columns);
    const showCellLabels = this.config.showCellLabels === true;

    const symbolType = resolveSymbolType(d3, this.config.iconShape);
    const iconSize = Math.pow(cellDim * 0.7, 2);
    const symbolPath = d3.symbol().type(symbolType).size(iconSize)();

    const iconGroups = this.svg
      .selectAll(".icon-cell")
      .data(icons)
      .enter()
      .append("g")
      .attr("class", "icon-cell")
      .attr(
        "transform",
        (d) =>
          `translate(${d.col * (cellWidth + gap) + cellWidth / 2},${
            (d.rows - 1 - d.row) * (cellHeight + gap) + cellHeight / 2
          })`
      );
    ...
    if (showCellLabels) {
      iconGroups
        ...
        .style("font-size", `${Math.max(6, cellDim * 0.22)}px`)
        .style("fill", "#3e3e3c")
