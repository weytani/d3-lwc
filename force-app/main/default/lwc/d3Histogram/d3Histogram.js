/**
 * D3 Histogram Lightning Web Component
 * Displays distribution of numeric values with automatic binning.
 * Supports configurable bin count, normal distribution overlay, and navigation on click.
 */
...
export default class D3Histogram extends NavigationMixin(LightningElement) {
    ...
    calculateStatistics() {
        ...
        if (n === 0) {
            this.statistics = { mean: 0, median: 0, stdDev: 0, count: 0, min: 0, max: 0 };
            return;
        }
    ...
    renderChart(containerWidth) {
        ...
        if (this.showNormalCurve && this.statistics.stdDev > 0) {
            this.drawNormalCurve(xScale, yScale, width, height);
        }
     ...
     */
    drawNormalCurve(xScale, yScale, width, height) {
        const d3 = this.d3;
        ...
        const binWidth = this.binData.length > 0 
            ? (this.binData[0].x1 - this.binData[0].x0) 
            : 1;
    ...
    handleBinClick(d) {
        ...
        this.dispatchEvent(new CustomEvent('binclick', {
            detail: {
                rangeStart: d.x0,
                rangeEnd: d.x1,
                count: d.length,
