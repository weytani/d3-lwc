...
// makes, so tests can assert a real lollipop head and the aggregated domain.
...
describe("d3LollipopChart GraphQL path", () => {
  ...
  it("renders the chart container and draws a real head when GraphQL aggregate data arrives", async () => {
    ...
    // a "lollipop-head" circle must have been appended.
    expect(
      d3Calls.some(
        (c) => c[0] === "attr" && c[1] === "class" && c[2] === "lollipop-head"
      )
    ).toBe(true);
  });
