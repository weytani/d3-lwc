// ABOUTME: Bundle-local data processing for the d3StackedBarChart standalone bundle.
// ABOUTME: Client-side validation, truncation, and single/two-field aggregation of records.
 ...
 * GraphQL wire, producing the [{label, value}] shape used for the no-series
 * (single-dimension) render path.
...
/**
 * Aggregates data by two group fields (label + series). Groups by the composite
 * (groupByField|seriesField) key and sums the value field per key, so duplicate
 * source rows for the same (label, series) collapse into one bucket — matching
 * the server-side two-field GROUP BY the structured GraphQL aggregate path
 * produces. Returns a flat array of { label, series, value } objects.
 * @param {Array} data - Array of records
