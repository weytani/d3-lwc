// ABOUTME: Bundle-local data processing for the d3StackedHorizontalBar standalone bundle.
// ABOUTME: Client-side validation, truncation, and single- or two-field (series) aggregation of records.
 ...
 * GraphQL wire, producing the [{label, value}] shape the chart renders as
 * simple bars (no series).
 ...
 * Groups by the composite (groupByField, seriesField) key and sums the
 * valueField per key with the chart's operation, so duplicate rows for the
 * same category/series are collapsed into one segment — this is the client-side
 * pivot+sum that keeps the free-text GraphQL path's numbers identical to the
 * server-side grouped aggregate.
 * @param {Array} data - Array of records
 * @param {String} groupByField - Primary grouping (category axis)
