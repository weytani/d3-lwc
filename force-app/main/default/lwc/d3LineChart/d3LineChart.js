...
export default class D3LineChart extends NavigationMixin(LightningElement) {
   ...
   * records are shaped into time-series points by dateField/valueField/seriesField.
   */
  @api graphqlQuery = "";
   ...
   * missing. A non-blank graphqlQuery overrides the structured builder. Line has
   * no server-side aggregate: the structured path always fetches raw records for
  ...
  async loadData() {
    ...
    // (structured record query or a free-text graphqlQuery) provides the data
