// ABOUTME: Bundle-local Salesforce GraphQL (v2) record-query builder and normalizer for the d3AreaChart bundle.
 ...
 * Used by both the structured record-query path (project the date/value/series
 * fields, then shape into a time series client-side) and the free-text
 * `graphqlQuery` override. When `objectApiName` is omitted or absent from the
 * payload, the first object key under `data.uiapi.query` is used — so an admin's
 * free-text query is accepted regardless of the object it targets. When `fields`
 * is omitted, every field present on the node is projected.
